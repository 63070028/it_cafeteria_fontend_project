...
const Item = ({ id, user_name, phone, status, result_money, timestamp, navigation, gotoOrderdetail,cancelDetail }) =>{
  ...
  return(
    ...
    <View style={styles.itemContainer}>
        <View style={styles.infoContainer}>
          ...
          <Text>หมายเหตุการยกเลิก: {cancelDetail}</Text>
          </View>
      </View>
  </View>
  </TouchableOpacity>
...
const CategoriesScreen = ({navigation}) => {
    ...
    todoRef.onSnapshot(
      querySnapshot =>{
        ...
        querySnapshot.forEach((doc) =>{
          const {user_name, phone, status, result_money, timestamp, user_id, orders, cancelDetail} = doc.data()
          if (doc.data().status == 'cancel' && user_data.id == doc.data().user_id) {
            users.push({
              id:doc.id,
              user_name,
              phone,
  ...
  const renderGridItem = (itemData) => {
    return (
      ...
      cancelDetail={itemData.item.cancelDetail}
      gotoOrderdetail={() =>{
